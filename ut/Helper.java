...
//        }
    ...
    public static long[][] loadDataValues(String filePath, int numRows, int numColumns) {
        long[][] data = new long[numRows][numColumns];
        ...
        try (BufferedReader file = new BufferedReader(new FileReader(filePath))) {
            ...
            while ((line = file.readLine()) != null) {
                ...
                // Skip header row if the first value is not a number
                if (rowIndex == 0) {
                    try {
                        Long.parseLong(values[0].trim());
                    } catch (NumberFormatException e) {
                        //  System.out.println("Skipping header row: " + Arrays.toString(values));
                        continue;
                    }
                }

                // Parse row data
                if (rowIndex < numRows && values.length >= numColumns) {
                    for (int colIndex = 0; colIndex < numColumns; colIndex++) {
                        try {
                            data[rowIndex][colIndex] = Long.parseLong(values[colIndex].trim());
                        } catch (NumberFormatException e) {
                            System.out.printf("Invalid number at row %d, column %d: %s%n",
                                    rowIndex, colIndex, values[colIndex]);
                            data[rowIndex][colIndex] = 0; // Default to 0 for invalid values
                        }
                    }
                    rowIndex++;
                }
